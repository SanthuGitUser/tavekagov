"""
Quick connectivity check for tnsectdemo.tn.gov.in (IAS transfers & postings).

Used by GitHub Actions before running IAS scrapers so unreachable hosts
...
from config import get_tn_ias_transfers_postings_source_url
...
def main() -> int:
    source_url = get_tn_ias_transfers_postings_source_url()
    ...
    try:
        ...
        print(f"tnsectdemo.tn.gov.in is not reachable from this runner: {exc}", file=sys.stderr)
        print(file=sys.stderr)
        print("GitHub-hosted runners are often blocked by tnsectdemo.tn.gov.in.", file=sys.stderr)
    ...
    print(f"tnsectdemo.tn.gov.in is reachable (HTTP {response.status_code}).")
    return 0
