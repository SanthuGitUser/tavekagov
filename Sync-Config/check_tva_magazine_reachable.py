"""
Quick connectivity check for tamildigitallibrary.in (Tamil Arasu magazine).

Used by GitHub Actions before running magazine scrapers so unreachable hosts
...
def main() -> int:
    source_url = get_tva_magazine_source_url()
    ...
    try:
        ...
        print(f"tamildigitallibrary.in is not reachable from this runner: {exc}", file=sys.stderr)
        print(file=sys.stderr)
        print("GitHub-hosted runners are often blocked by tamildigitallibrary.in.", file=sys.stderr)
    ...
    print(f"tamildigitallibrary.in is reachable (HTTP {response.status_code}).")
    return 0
