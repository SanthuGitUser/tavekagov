"""
Quick connectivity check for www.tn.gov.in.

Used by GitHub Actions before running tn.gov.in scrapers so unreachable hosts
...
from config import get_tn_gov_base_url
...
def main() -> int:
    base_url = get_tn_gov_base_url()
    session = requests.Session()
    session.headers.update(_DEFAULT_HEADERS)

    try:
        response = session.get(base_url, timeout=_PREFLIGHT_TIMEOUT)
        response.raise_for_status()
    except requests.RequestException as exc:
        print(f"tn.gov.in is not reachable from this runner: {exc}", file=sys.stderr)
        print(file=sys.stderr)
        print("GitHub-hosted runners are often blocked by tn.gov.in.", file=sys.stderr)
    ...
    print(f"tn.gov.in is reachable (HTTP {response.status_code}).")
    return 0
