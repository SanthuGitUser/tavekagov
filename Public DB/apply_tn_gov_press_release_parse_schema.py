"""Add parsed-title metadata columns for tn_gov_press_releases."""

from __future__ import annotations

import psycopg2

from config import get_database_url

DDL = """
alter table public.tn_gov_press_releases
    add column if not exists minister_name text,
    add column if not exists department_name text,
    add column if not exists minister_id bigint references public.tn_ministers (id),
    add column if not exists department_id bigint references public.tn_dept (id),
    add column if not exists district_id bigint references public.tn_districts (id),
    add column if not exists title_parsed boolean not null default false,
    add column if not exists parse_confidence text,
    add column if not exists minister_match_confidence text,
    add column if not exists department_match_confidence text,
    add column if not exists district_match_confidence text;

create index if not exists tn_gov_press_releases_minister_id_idx
    on public.tn_gov_press_releases (minister_id);

create index if not exists tn_gov_press_releases_department_id_idx
    on public.tn_gov_press_releases (department_id);

create index if not exists tn_gov_press_releases_district_id_idx
    on public.tn_gov_press_releases (district_id);

create index if not exists tn_gov_press_releases_title_parsed_idx
    on public.tn_gov_press_releases (title_parsed)
    where title_parsed = false;
"""


def main() -> int:
    database_url = get_database_url()
    if not database_url:
        print("DATABASE_URL / SUPABASE_DB_PASSWORD is not configured.")
        return 1

    with psycopg2.connect(database_url) as conn:
        conn.autocommit = True
        with conn.cursor() as cur:
            cur.execute(DDL)

    print("tn_gov_press_releases parse-title schema is ready.")
    return 0


if __name__ == "__main__":
    raise SystemExit(main())
